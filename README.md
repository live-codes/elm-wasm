...
- **The compiler binary ships ~13 `elm/*` packages.** Importing one of them needs no download — it only has to be declared as a dependency, since its precompiled artifacts are already there. Anything else is fetched on demand; packages with native kernel code can only be official `elm/*` ones anyway.
